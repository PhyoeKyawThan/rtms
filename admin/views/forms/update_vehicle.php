<?php
include (__DIR__ . '/../../includes/vehicles/get_vehicle_by_id.php');
...
<body class="h-screen">
    <div class="h-full overflow-scroll bg-slate-300 flex justify-center p-10">
        <form action="/action/update_vehicle?id=<?php echo $data['id'] ?>" method="post" class="h-fit w-3/5 bg-white p-9 rounded-2xl">
            ...
            <div class="mb-2">
                ...
                <input type="text" name="vehicle-number" id="vehicle-number"
                    value="<?php echo $data['vehicle_number']; ?>"
            ...
            <div class="mb-2">
                ...
                <input type="text" name="nrc" value="<?php echo $data['nrc']; ?>" id="nrc"
            ...
            <div class="mb-2">
                ...
                <textarea name="address" id="address" class="border mt-3 border-slate-300 rounded-xl p-2 ml-5 w-full"
                    placeholder="နေရပ်လိပ်စာထည့်ရန်..." required><?php echo $data["address"]; ?></textarea>
            </div>
            <div class="mb-2">
                <label class="block font-bold" for="vehicle-brand">အမျိုးအမည် *</label>
                <input type="text" name="vehicle-brand" value="<?php echo $data['vehicle_brand']; ?>" id="vehicle-brand"
            ...
            <div class="mb-2">
                ...
                <select name="vehicle-type" class="border mt-3 border-slate-300 rounded-xl p-2 ml-5 w-full" required>
                    <option value="ဆိုင်ကယ်" <?php echo isset($data["vehicle_type"]) && $data["vehicle_type"] == "ဆိုင်ကယ်" ? "selected" : ""; ?>>ဆိုင်ကယ်</option>
                    <option value="သုံးဘီး" <?php echo isset($data["vehicle_type"]) && $data["vehicle_type"] == "သုံးဘီး" ? "selected" : ""; ?>>သုံးဘီး</option>
                    <option value="ထော်လာဂျီ" <?php echo isset($data["vehicle_type"]) && $data["vehicle_type"] == "ထော်လာဂျီ" ? "selected" : ""; ?>>ထော်လာဂျီ</option>
                    <option value="ကား" <?php echo isset($data["vehicle_type"]) && $data["vehicle_type"] == "ကား" ? "selected" : ""; ?>>ကား</option>
                </select>

            </div>
            <div class="mb-2">
                <label class="block font-bold" for="unique-no">ဘောင်အမှတ်</label>
                <input type="text" name="unique-no" id="unique-no" value="<?php echo $data['unique_no']; ?>"

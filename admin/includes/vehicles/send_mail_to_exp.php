...
if (isset($_POST['mail']) && isset($_POST['reply-content']) && isset($_POST["subject"])) {
    if (send_mail($_POST["mail"], $_POST["subject"], $_POST["reply-content"])) {
        ...
        echo json_encode(array(
            "status"=> "success",
            "message"=> "Warning Mail has been sent successfully!"
        ));
        // header("Location: /home?p=expired_vehicles?message=Warning Mail has been sent successfully!");
        // exit;
    } else {
        http_response_code(200);
        header("Content-Type: application/json");
        ...
        // exit;
        // http_response_code(200);
        // header("Location: /home?p=expired_vehicles?error=Error while sending mail");
        // exit;
    }
}
